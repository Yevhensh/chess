...
public class MovementHandler implements EventHandler<MouseEvent> {
  ...
  private Selected resolveClickedSelection(Node node) {
    ...
    if (node instanceof ImageView imageView) {
      return layoutManager.getSelectedByDeckPosition(
          (int) imageView.getX() - IMAGE_VIEW_OFFSET, (int) imageView.getY() - IMAGE_VIEW_OFFSET);
    }
    return null;
  }
  ...
  private void handleExistingSelection(Selected clicked) {
    ...
    if (isValidMoveTarget(clicked, selectedFigure, fromIndex)) {
      boolean isLegal;
      if (selectedFigure instanceof chess.main.sample.figures.instances.King
          && Math.abs(ChessUtils.getCol(fromIndex) - ChessUtils.getCol(clicked.index())) == 2) {
        isLegal =
            deckManager.isCastlingLegal(fromIndex, clicked.index(), selectedFigure.getPosition());
      } else {
  ...
  private boolean isAllyPiece(Selected clicked, Figure selectedFigure) {
    Figure clickedFigure = clicked.selected();
    return clickedFigure != null
        && clickedFigure.getPosition().equals(selectedFigure.getPosition());
  }
  ...
  private boolean isValidMoveTarget(Selected clicked, Figure selectedFigure, int fromIndex) {
    ...
    List<Integer> availableMoves =
        selectedFigure.getAllAvailableMovements(
            positions, fromIndex, gameState.getHistoryManager().getLastMove());
    ...
    List<Integer> availableMoves =
        selected
            ...
            .getAllAvailableMovements(
                positions, selected.index(), gameState.getHistoryManager().getLastMove());
    for (int moveIndex : availableMoves) {
      boolean isLegal;
      if (selected.selected() instanceof chess.main.sample.figures.instances.King
          && Math.abs(ChessUtils.getCol(selected.index()) - ChessUtils.getCol(moveIndex)) == 2) {
        isLegal =
            deckManager.isCastlingLegal(
                selected.index(), moveIndex, selected.selected().getPosition());
      } else {
        isLegal =
            deckManager.isMoveLegal(selected.index(), moveIndex, selected.selected().getPosition());
      }

      if (isLegal) {
        boolean isCapture =
            deckManager.isOppositeFigureOnDeckCell(
                positions, moveIndex, selected.selected().getPosition());
  ...
  private String buildStatusMessage(Position currentSide) {
    int kingIndex =
        currentSide == Position.WHITE ? storage.getWhiteKingIndex() : storage.getBlackKingIndex();
