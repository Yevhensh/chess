...
public class DeckLayoutManager {
  ...
  private void renderCoordinates() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      fileLabel.setLayoutX(
          START_FROM_BORDER_WIDTH + i * RECTANGLE_DIMENSION + RECTANGLE_DIMENSION / 2.0 - 5);
      ...
      rankLabel.setLayoutY(
          START_FROM_BORDER_WIDTH + i * RECTANGLE_DIMENSION + RECTANGLE_DIMENSION / 2.0 - 10);
  ...
  public void showMoveIndicator(int index, boolean isCapture) {
    ...
    if (isCapture) {
      Circle ring =
          new Circle(
              x + RECTANGLE_DIMENSION / 2.0,
              y + RECTANGLE_DIMENSION / 2.0,
              RECTANGLE_DIMENSION / 2.0 - 5);
  ...
  public void clearIndicators(int index) {
    ...
    List<Node> indicators = nodes.stream().filter(node -> node instanceof Circle).toList();
  ...
  private ImageView createPieceImageView(int cellX, int cellY, Figure figure) {
    Image image =
        new Image(getClass().getClassLoader().getResourceAsStream(figure.getFilenamePath()));
