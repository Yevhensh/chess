...
public class DeckManager {
    ...
    private List<Integer> getAttacksForFigure(Map<Integer, Figure> positions, Map.Entry<Integer, Figure> entry) {
        ...
        return switch (figure) {
            ...
            default -> figure.getAllAvailableMovements(positions, entry.getKey(), gameState.getHistoryManager().getLastMove());
        };
    }
    ...
    public boolean hasPieceMoved(int index, Figure figure) {
        return gameState.getHistoryManager().getMoves().stream()
                .anyMatch(m -> m.fromIndex() == index || (m.movedFigure() == figure && m.fromIndex() != index));
    }
    ...
    public boolean isCastlingLegal(int fromInd, int toInd, Position side) {
        ...
        boolean kingMoved = gameState.getHistoryManager().getMoves().stream()
                .anyMatch(m -> m.movedFigure() instanceof King && m.movedFigure().getPosition() == side);
        ...
        if (!(rook instanceof chess.main.sample.figures.instances.Rok) || rook.getPosition() != side) return false;

        // Rook must not have moved
        boolean rookMoved = gameState.getHistoryManager().getMoves().stream()
                .anyMatch(m -> m.fromIndex() == rookInd || (m.movedFigure() == rook && m.fromIndex() != rookInd));
        ...
        for (int c = start + 1; c < end; c++) {
            if (!ChessUtils.isEmpty(storage.getPositionsContainer(), ChessUtils.getIndex(row, c))) return false;
        }
    ...
    private boolean isSquareAttacked(int index, Position side) {
        ...
        return getAllOppositeSiteAttacks(storage.getPositionsContainer(), oppositePosition).contains(index);
    }
    ...
    public void makeTurn(int fromInd, int toInd) {
        ...
        if (figure instanceof Pawn && captured == null && ChessUtils.getCol(fromInd) != ChessUtils.getCol(toInd)) {
            isEnPassant = true;
        ...
        boolean isCastling = (figure instanceof King && Math.abs(ChessUtils.getCol(fromInd) - ChessUtils.getCol(toInd)) == 2);
        ...
        gameState.getHistoryManager().addMove(
            new Move(fromInd, toInd, positionsContainer.get(fromInd), captured, capturedIndex, wasPromoted, isCastling, isEnPassant),
            getBoardSnapshot(),
    ...
    public boolean hasAnyLegalMoves(Position side) {
        ...
        return currentPositions.entrySet().stream()
                ...
                .anyMatch(entry -> {
                    ...
                    return entry.getValue().getAllAvailableMovements(currentPositions, fromInd, gameState.getHistoryManager().getLastMove()).stream()
    ...
    public boolean isInsufficientMaterial() {
        ...
        if (figures.size() == 4) {
            ...
            if (bishops.size() == 2 && bishops.get(0).getPosition() != bishops.get(1).getPosition()) {
                ...
                List<Integer> bishopIndices = positions.entrySet().stream()
                ...
                return (ChessUtils.getRow(b1) + ChessUtils.getCol(b1)) % 2 == (ChessUtils.getRow(b2) + ChessUtils.getCol(b2)) % 2;
            }
        }
